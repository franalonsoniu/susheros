...
t1, t3, t4, t5  = st.columns((3.5,4,2,1))
...
cur.execute("""SELECT *
               FROM susheros_app_dev
               WHERE filter_date IN ('2022-11-01','2022-12-01','2023-01-01')
...
#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-
    ...
    with col2:
        ...
        fig.update_traces(marker_color='#E74C3C', marker_line_color='#000000', marker_line_width=1)
...
######################################### Desglose Diario ##############################################
    ...
    des.rename(columns = {'Period':'Fecha', input_rut:'Monto'}, inplace = True)
    des["Repeat"] = des['year'].astype(str) +","+ des['mes'].astype(str)
    des.loc[:, 'Monto'] ='$'+des['Monto'].map('{:,.0f}'.format)

    # st.bar_chart(data=des, x='Fecha', y='Monto')
...
############################################# Fin del Loop #############################################
    ...
    def highlight_greaterthan(x):
        if x['Monto a Pagar']!= '$0':
            return ['background-color: ']*9
        else:
            return ['background-color: #F56262']*9
